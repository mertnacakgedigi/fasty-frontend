...
export default function SearchMultipleCity() {
  ...
  const handleSelectCity = (city: ICity) => {
    ...
    if (isCitySelected(city)) {
      ...
      setSelectedCities(prevCities => prevCities.filter(selectedCity => selectedCity.id !== city.id));
    } else {
      ...
      <Combobox as="div" onChange={handleSelectCity}>
        <div className="relative mt-2 flex ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-red-900 flex-wrap items-center bg-white rounded-md shadow-sm text-sm">
          {selectedCities.map(city => (
            <span key={city.id} className="inline-flex items-center rounded-full bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 mr-2 m-1">
              {city.name}, {city.stateCode}
              <button onClick={() => handleRemoveCity(city.id)} className="ml-2 text-red-900">x</button>
            </span>
          ))}
          <Combobox.Input
            value={query}
